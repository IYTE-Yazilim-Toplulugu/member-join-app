...
import { sendWelcomeEmail } from "@/app/functions/sendEmail";
...
export const POST = async (request: NextRequest) => {
    ...
    try {
        ...
        // Check User:
        const user = await OutlierModel.findOne({ $or: [
            { email: data.email },
        ...
        await OutlierModel.create({ ...data, token: token, password: hashedPassword });
